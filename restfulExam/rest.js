...
app.post('/user/login',function(req,res){
	...
	connection.query('select id from user where user_id = ? and password = ?', [req.body.user_id, hash], 
		function(err, results, fields) {
			if(err) {
				res.send(JSON.stringify(err));
			}else {
					...
					jwt.encode(secret, settingAddHeaders, 
						function(err, token){
							if(err) {
								...
								connection.query('insert into user_login(user_real_id, token) values(?,?)',
									[results[0].id, tokens[2]] , 
									function(err,result) {
										if(err) {
											res.send(JSON.stringify(err));
										} else {
